...
public class ExploreManager {
	...
	public ExploreRespond chunkRespond(Player player, Chunk chunk) {
		return chunkRespond(Jobs.getPlayerManager().getJobsPlayer(player).getUserId(), chunk.getWorld().getName(),
	...
	public ExploreRespond chunkRespond(int playerId, String world, int x, int z) {
		...
		if (chunk == null) {
			Jobs.getPluginLogger().warning("Creating new chunk");
	...
	public void load(ResultSet res) {
		try {
			...
			Jobs.getPluginLogger().warning("loading id " + id);

			Map<String, ExploreRegion> eRegions = worlds.getOrDefault(jobsWorld.getName(),
					new HashMap<String, ExploreRegion>());
